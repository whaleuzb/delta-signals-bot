...
70. **To'lovli kirish (paywall) — guruh egasi o'z guruhiga Stars orqali obuna sotadi.**
    Ikkita kirish yo'li BIR VAQTDA ishlaydi:
    - **A) Umumiy havola** — `create_chat_invite_link(creates_join_request=True)`.
      Havolaning o'zi hech kimni ichkariga kiritmaydi, faqat SO'ROV yuboradi;
      `ChatJoinRequestHandler(on_join_request)` obunani tekshiradi — bor bo'lsa
      darhol `approve_chat_join_request`, yo'q bo'lsa DM'ga taklif (so'rov
      OCHIQ qoladi, chunki odam aynan shu daqiqada to'lashi mumkin; botga
      umuman yozolmaydigan bo'lsa — `Forbidden` — o'shandagina rad etiladi,
      aks holda so'rov abadiy osilib qolardi).
    - **B) Bir martalik havola** — to'lovdan keyin `member_limit=1,
      expire_date=+1 soat`. Kutib turgan so'rov bo'lmasa shu ishlatiladi.
    - **MUHIM**: guruh YOPIQ bo'lishi shart — ochiq guruhda Telegram oddiy
      "Join" tugmasini ko'rsatib so'rovni chetlab o'tadi (PTB hujjatida ham
      shu haqda ogohlantirish bor).
    To'lov — Telegram Stars (`send_invoice(currency="XTR", provider_token=None)`),
    provayder/shartnoma kerak emas. `PreCheckoutQueryHandler` pul yechilishidan
    OLDIN tekshiradi (narx invoysdan keyin o'zgarmaganmi), shuning uchun
    qaytarib berish muammosi umuman tug'ilmaydi.
    - Stars BOT EGASINING balansiga tushadi, guruh egasiga emas — shu sabab
      panelda `config.PLATFORM_FEE_PCT` ayirilgan "sizga tegishli" summa
      alohida ko'rsatiladi (qo'lda o'tkazish uchun).
    - `star_payments.charge_id` UNIQUE — Telegram bir xil to'lov xabarini
      qayta yuborsa (`SuccessfulPayment`) obuna ikkinchi marta uzaytirilmaydi.
    - `memberships.expires_at` — `extend_membership()` muddati tugamagan
      bo'lsa MAVJUD sanaga qo'shadi (erta to'laganda kun yo'qolmasin), tugagan
      bo'lsa hozirdan boshlaydi.
    - `subs_job` (6 soatda bir) ikki ishni qiladi: muddati `SUB_REMIND_DAYS`
      (standart 3) kun qolganlarga bir martalik eslatma (`reminded` bayrog'i
      bilan himoyalangan) va muddati o'tganlarni `ban_chat_member` +
      darhol `unban_chat_member(only_if_banned=True)` bilan chiqarish — bu
      "chiqarib yuborish, lekin qora ro'yxatga QO'YMASLIK": qayta to'lasa
      yana kira oladi. Guruh egasi va super-adminlar hech qachon chiqarilmaydi.
    - `gate()` (majburiy obuna tekshiruvi) to'lov oqimini ATAYLAB chetlab
      o'tadi (`pay:` callback, pre_checkout_query, successful_payment) —
      aks holda eng yomon holatda pul yechilib, obuna yozilmay qolishi mumkin
      edi.
    - Panel: `/tolov` yoki bosh menyudagi "💳 To'lovli kirish" (faqat guruh
      workspace egasiga ko'rinadi). Yoqishdan oldin bot huquqlari
      tekshiriladi (`can_invite_users` shart, `can_restrict_members` bo'lmasa
      ogohlantirish bilan yoqiladi — aks holda chiqarish keyin sukut saqlab
      ishlamay qolardi).
    - Tekshirildi (haqiqiy Postgres, soxta bot): so'rov→taklif, to'lov→obuna+
      xabar (100⭐, komissiya 10% → 90 net), takroriy charge_id→o'tkazib
      yuborildi, obunachi so'rov yuborsa→darhol tasdiq, muddat tugashi→
      ban+unban+xabar, eslatma bir marta. `bot.main()` to'liq handler
      ro'yxati bilan xatosiz quriladi (5 ta job, jumladan yangi `subs_job`).
      `test_tracker.py` 9/9.
